...
# Recovery time for anti-thrashing: after N ineffective compressions, pause
# compression for this many seconds before allowing re-evaluation. This prevents
# infinite compression loops on models that produce marginal gains repeatedly.
# (Fixes issue #14694)
_ANTI_THRASH_RECOVERY_SECONDS = 3600
...
class ContextCompressor(ContextEngine):
    ...
    def update_model(
        ...
        self.threshold_tokens = max(
            ...
            MINIMUM_CONTEXT_LENGTH,
            512,  # Absolute floor: never compress below 512 tokens (fixes #14690)
        )
    ...
    def __init__(
        ...
        # FIX (#14690): also enforce an absolute floor of 512 tokens so that
        # models with tiny context windows (e.g. 2K, 4K) still have enough
        # context space for the compressor to operate effectively.  Without
        # this floor, threshold_tokens can be 100 tokens or less, causing
        # the compressor to trigger too early and lose too much context.
        self.threshold_tokens = max(
    ...
    def should_compress(self, prompt_tokens: int = None) -> bool:
        ...
        if self._ineffective_compression_count >= 2:
            ...
            elapsed = time.monotonic() - self._last_ineffective_compression_time
            if elapsed < _ANTI_THRASH_RECOVERY_SECONDS:
                if not self.quiet_mode:
                    remaining = int(_ANTI_THRASH_RECOVERY_SECONDS - elapsed)
                    logger.warning(
                        "Compression skipped — last %d compressions saved <10%% each. "
                        "Re-evaluating in %d seconds. "
                ...
                # Cooldown expired — reset counter and re-evaluate
                self._ineffective_compression_count = 0
                if not self.quiet_mode:
                    logger.info(
                        "Anti-thrashing cooldown expired (%d seconds). "
                        "Re-evaluating compression eligibility.",
